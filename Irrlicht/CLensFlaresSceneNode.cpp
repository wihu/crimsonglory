...
CLensFlareSceneNode::CLensFlareSceneNode( ISceneNode* parent, ISceneManager* mgr, s32 id, const core::vector3df& position, bool lockXPos ) : ISceneNode( parent, mgr, id, position )
{
...
int CLensFlareSceneNode::getMaterialCount()
{
    return 1;
}
...
void CLensFlareSceneNode::OnPreRender()
{
    if ( IsVisible )
    {
      ...
      {
        //line3d<f32> line( SceneManager->getActiveCamera()->getAbsolutePosition(), getAbsolutePosition() );
        //if ( SceneManager->getSceneCollisionManager()->getSceneNodeFromRayBB( line ) )
        {
