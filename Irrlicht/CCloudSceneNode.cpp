...
video::SMaterial& CCloudSceneNode::getMaterial(s32 i)
{
    return Material;
}
...
s32 CCloudSceneNode::getMaterialCount()
{
    return 1;
}
...
void CCloudSceneNode::makeParticles() 
{
    ...
    u32 ufar = int(SceneManager->getActiveCamera()->getFarValue() *10000);

    if (SkyClouds)
        pos = getPosition()+campos-(campos*SkyMoveFactor);
    else
        pos = getPosition();
    ...
    for (i=0; i < ParticleCount; i++)
    {
        ...
        f32 td = campos.getDistanceFrom(tmppos);
        u32 tmpdist = 10000 * td;
...
s16 CCloudSceneNode::makeChildClouds(SCloudParticle &parent,s16 start,core::matrix4 &m, core::vector3df &campos )
{
    ...
    for(count=0;count<parent.children;count++)
    {
        ...
        // set distance from camera
        f32 td = campos.getDistanceFrom(Particles[c].pos);
...
//! pre render event
void CCloudSceneNode::OnPreRender()
{
    if (IsVisible)
    {
...
void CCloudSceneNode::makeRandomCloud(s16 count)
{
    ...
    for (i=1; i<count+25; i++) 
    {
        ...
        ParticleData[i].pos = core::vector3d<f32>(0,0,100);
        ...
        // set the size
        ParticleData[i].size = core::dimension2d<f32>(2+f,2+f);
        // distance to dark spot
        int f2 = int(rand())%128;
        f2+=1;
        ParticleData[i].color = video::SColor(255,255-f2,255-f2,255-f2);
        ParticleData[i].children = 0;
        if (rand()/500 > 20) ParticleData[i].children = int(rand()) %3;
        int j = int(rand()) %10;
