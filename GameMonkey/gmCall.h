...
class gmCall
{
  public:
    ...
    GM_FORCEINLINE bool BeginGlobalFunction( gmMachine* a_machine, const char* a_funcName,
                                             const gmVariable& a_this = gmVariable::s_null, bool a_delayExecuteFlag = false )
    {
        GM_ASSERT( a_machine );
    ...
    /// \param a_delayExecuteFlag Set true if you want function thread to not execute now.
    /// \return true on sucess, false if function was not found.
    GM_FORCEINLINE bool BeginTableFunction( gmMachine* a_machine, const char* a_funcName, gmTableObject* a_tableObj,
                                            const gmVariable& a_this = gmVariable::s_null, bool a_delayExecuteFlag = false )
    {
        GM_ASSERT( a_machine );
    ...
    GM_FORCEINLINE bool BeginFunction( gmMachine* a_machine, gmFunctionObject* a_funcObj,
                                       const gmVariable& a_thisVar = gmVariable::s_null, bool a_delayExecuteFlag = false )
    {
        GM_ASSERT( a_machine );   
        GM_ASSERT( a_funcObj );
