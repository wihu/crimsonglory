...
static int GM_CDECL gmfStringMid( gmThread* a_thread )
{
    ...
    if ( !gmGetFloatOrIntParamAsInt( a_thread, 0, first ) )
    {
      return GM_EXCEPTION;
    }
    if ( !gmGetFloatOrIntParamAsInt( a_thread, 1, count ) )
    {
      return GM_EXCEPTION;
    }
...
static int GM_CDECL gmStringFind( gmThread* a_thread )
{
    ...
    if ( ( strObj->GetLength() == 0 ) || ( startOffset > strObj->GetLength() ) || ( startOffset < 0 ) )
    {
...
{
    if ( a_operands[0].m_type != GM_STRING || a_operands[1].m_type != GM_INT )
    {
      a_operands->Nullify();
      return;
    }
