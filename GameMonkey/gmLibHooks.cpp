...
gmLibHooks::gmLibHooks( gmStream& a_stream, const char* a_source ) : m_allocator( 1, GMCODETREE_CHAINSIZE )
{
    m_stream = &a_stream;
    m_source = a_source;
...
gmFunctionObject* gmLibHooks::BindLib( gmMachine& a_machine, gmStream& a_stream, const char* a_filename )
{
    ...
    if ( ( a_stream.Read( &header, sizeof( header ) ) != sizeof( header ) ) || header.m_id != ID_gml0 )
    {
      goto done;
    }
    ...
    if ( a_stream.Read( &strings, sizeof( strings ) ) != sizeof( strings ) )
    {
      goto done;
    }
    stringTable = GM_NEW( char[strings.m_size] );
    if ( a_stream.Read( stringTable, strings.m_size ) != strings.m_size )
    {
      goto done;
    }
    ...
    if ( header.m_scOffset && a_machine.GetDebugMode() )
    {
      ...
      if ( a_stream.Read( &source, sizeof( source ) ) != sizeof( source ) )
      {
        goto done;
      }
      sourceCode = GM_NEW( char[source.m_size] );
      if ( a_stream.Read( sourceCode, source.m_size ) != source.m_size )
      {
        goto done;
      }
    ...
    if ( a_stream.Read( &numFunctions, sizeof( numFunctions ) ) != sizeof( numFunctions ) )
    {
      goto done;
    }
    ...
    for ( i = 0; i < numFunctions; ++i )
    {
      if ( ( a_stream.Read( &function, sizeof( function ) ) != sizeof( function ) ) || function.m_func != ID_func )
      {
        goto done;
      }
      ...
      if ( a_stream.Read( byteCode, function.m_byteCodeLen ) != function.m_byteCodeLen )
      {
        goto done;
      }
      ...
      for ( ; instruction < end; )
      {
        switch ( *( instruction32++ ) )
        {
          ...
          case BC_PUSHINT :
          case BC_PUSHFP :
            instruction += sizeof( gmfloat ); break;

          case BC_CALL :
          case BC_GETLOCAL :
          case BC_SETLOCAL :
            instruction += sizeof( gmuint32 ); break;
      ...
      if ( debug )
      {
        ...
        // debug name
        if ( a_stream.Read( &stringOffset, sizeof( stringOffset ) ) != sizeof( stringOffset ) )
        {
          goto done;
        }
        ...
        if ( a_stream.Read( &lineInfoCount, sizeof( lineInfoCount ) ) != sizeof( lineInfoCount ) )
        {
          goto done;
        }
        ...
        for ( j = 0; j < lineInfoCount; ++j )
        {
          ...
          if ( a_stream.Read( &libLineInfo, sizeof( libLineInfo ) ) != sizeof( libLineInfo ) )
          {
            goto done;
          }
