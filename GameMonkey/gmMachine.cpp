...
class gmHooks : public gmCodeGenHooks
{
  public:
    ...
    gmFunctionObject* GetRootFunction()
    {
...
void GM_CDECL gmMachine::ScanRootsCallBack( gmMachine* a_machine, gmGarbageCollector* a_gc )
{
    ...
    {
      s_machineCallback( a_machine, MC_COLLECT_GARBAGE, NULL );
    }
    ...
    for ( tit = a_machine->m_runningThreads.GetFirst(); a_machine->m_runningThreads.IsValid( tit ); tit = a_machine->m_runningThreads.GetNext( tit ) )
    {
      tit->GCScanRoots( a_machine, a_gc );
    }
    for ( tit = a_machine->m_blockedThreads.GetFirst(); a_machine->m_blockedThreads.IsValid( tit ); tit = a_machine->m_blockedThreads.GetNext( tit ) )
    {
      tit->GCScanRoots( a_machine, a_gc );
    }
    for ( tit = a_machine->m_sleepingThreads.GetFirst(); a_machine->m_sleepingThreads.IsValid( tit ); tit = a_machine->m_sleepingThreads.GetNext( tit ) )
    {
      tit->GCScanRoots( a_machine, a_gc );
    }
    for ( tit = a_machine->m_exceptionThreads.GetFirst(); a_machine->m_exceptionThreads.IsValid( tit ); tit = a_machine->m_sleepingThreads.GetNext( tit ) )
    {
      tit->GCScanRoots( a_machine, a_gc );
    }
...
gmMachine::gmMachine() : m_threads( 128 ), m_memStringObj( sizeof( gmStringObject ), GMMACHINE_STRINGCHUNKSIZE ), m_memTableObj( sizeof( gmTableObject ), GMMACHINE_TBLCHUNKSIZE ), m_memFunctionObj( sizeof( gmFunctionObject ), GMMACHINE_OBJECTCHUNKSIZE ), m_memUserObj( sizeof( gmUserObject ), GMMACHINE_OBJECTCHUNKSIZE ), m_memStackFrames( sizeof( gmStackFrame ), GMMACHINE_STACKFCHUNKSIZE ), m_strings( GMMACHINE_STRINGHASHSIZE ), m_blocks( 64 ), m_cppOwnedGMObjs( GMMACHINE_CPPOWNEDGMOBJHASHSIZE )
{
    m_line = NULL;
    m_call = NULL;
    m_return = NULL;
...
gmThread* gmMachine::CreateThread( const gmVariable& a_this, const gmVariable& a_function, int* a_threadId )
{
    ...
    if ( thread->PushStackFrame( 0, 0 ) == gmThread::RUNNING )
    {
      return thread;
    }
    return NULL;
...
int gmMachine::Sys_Block( gmThread* a_thread, int m_numBlocks, const gmVariable* a_blocks )
{
    ...
    while ( signal )
    {
      ...
      {
        if ( a_blocks[i].m_type == signal->m_signal.m_type && a_blocks[i].m_value.m_ref == signal->m_signal.m_value.m_ref )
        {
          // remove the signal
...
void gmMachine::ForEachThread( gmThreadIteratorCallback a_callback, void* a_context )
{
    ...
    for ( it = m_runningThreads.First(); it; )
    {
      ...
      ++it;
      if ( !a_callback( thread, a_context ) )
      {
        return;
      }
    }
...
{
    if ( a_thread->GetState() == ( gmThread::State )a_to )
    {
      return;
    }
    switch ( a_thread->GetState() )
    {
      ...
      case gmThread::SLEEPING :
        m_sleepingThreads.Remove( a_thread ); break;
      case gmThread::KILLED :
        m_killedThreads.Remove( a_thread ); break;
      case gmThread::EXCEPTION :
        m_exceptionThreads.Remove( a_thread ); break;
      default :
        GM_ASSERT( 0 ); break;
    }
    switch ( a_to )
    {
      case gmThread::RUNNING :
        m_runningThreads.InsertLast( a_thread ); break;
      case gmThread::BLOCKED :
        m_blockedThreads.InsertFirst( a_thread ); break;
      case gmThread::EXCEPTION :
        m_exceptionThreads.InsertFirst( a_thread ); break;
      case gmThread::SLEEPING :
        {
            ...
            if ( !inserted )
            {
              m_sleepingThreads.InsertLast( a_thread );
            }
            break;
        }
      case gmThread::KILLED :
...
void gmMachine::Presize( int a_pool8, int a_pool16, int a_pool24, int a_pool32, int a_pool64, int a_pool128, int a_pool256, int a_pool512, int a_gmStringObj, int a_gmTableObj, int a_gmFunctionObj, int a_gmUserObj, int a_gmStackFrame )
{
    m_fixedSet.Presize( a_pool8, a_pool16, a_pool24, a_pool32, a_pool64, a_pool128, a_pool256, a_pool512 );
    if ( a_gmStringObj )
    {
      m_memStringObj.Presize( a_gmStringObj );
    }
    if ( a_gmTableObj )
    {
      m_memTableObj.Presize( a_gmTableObj );
    }
    if ( a_gmFunctionObj )
    {
      m_memFunctionObj.Presize( a_gmFunctionObj );
    }
    if ( a_gmUserObj )
    {
      m_memUserObj.Presize( a_gmUserObj );
    }
    if ( a_gmStackFrame )
    {
      m_memStackFrames.Presize( a_gmStackFrame );
    }
}
...
bool gmMachine::CollectGarbage( bool a_forceFullCollect )
{
    ...
    if ( m_gcEnabled && ( a_forceFullCollect || ( GetCurrentMemoryUsage() > m_desiredByteMemoryUsageHard ) ) )
    {
      ...
      if ( ++m_mark == GM_MARK_PERSIST )
      {
        m_mark = GM_MARK_START + 1;
      }

      // call the gc callback
      if ( s_machineCallback )
      {
        s_machineCallback( this, MC_COLLECT_GARBAGE, ( void* )&m_mark );
      }
      ...
      // iterate over all threads and mark the stacks.
      for ( tit = m_runningThreads.GetFirst(); m_runningThreads.IsValid( tit ); tit = m_runningThreads.GetNext( tit ) )
      {
        tit->Mark( m_mark );
      }
      for ( tit = m_blockedThreads.GetFirst(); m_blockedThreads.IsValid( tit ); tit = m_blockedThreads.GetNext( tit ) )
      {
        tit->Mark( m_mark );
      }
      for ( tit = m_sleepingThreads.GetFirst(); m_sleepingThreads.IsValid( tit ); tit = m_sleepingThreads.GetNext( tit ) )
      {
        tit->Mark( m_mark );
      }
      for ( tit = m_exceptionThreads.GetFirst(); m_exceptionThreads.IsValid( tit ); tit = m_exceptionThreads.GetNext( tit ) )
      {
        tit->Mark( m_mark );
      }
...
unsigned int gmMachine::GetSystemMemUsed() const
{
    ...
    for ( tit = m_killedThreads.GetFirst(); m_killedThreads.IsValid( tit ); tit = m_killedThreads.GetNext( tit ) )
    {
      total += tit->GetSystemMemUsed();
    }
    for ( tit = m_exceptionThreads.GetFirst(); m_exceptionThreads.IsValid( tit ); tit = m_exceptionThreads.GetNext( tit ) )
...
gmHooks::gmHooks( gmMachine* a_machine, const char* a_source, const char* a_filename ) : m_machine( a_machine ), m_source( a_source ), m_filename( a_filename )
{
