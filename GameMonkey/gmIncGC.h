...
#if GM_GC_DEBUG
enum { GM_GC_DEBUG_COL_INVALID, //0
...
class gmGCObjBase
{
  public:
    ...
    inline void SetColor( int a_color )
    {
        m_color = ( char ) a_color;
    }
    inline int GetColor()
    {
        return ( int ) m_color;
    }
    inline gmGCObjBase* GetPrev() const
    {
        return m_prev;
    }
    inline void SetPrev( gmGCObjBase* a_prev )
    {
        m_prev = a_prev;
    }
    inline gmGCObjBase* GetNext() const
    {
        return m_next;
    }
    inline void SetNext( gmGCObjBase* a_next )
    {
        m_next = a_next;
    }

    inline char GetPersist()
    {
        return m_persist;
    }
    inline void SetPersist( bool a_flag )
    {
        m_persist = a_flag;
    }
...
class gmGarbageCollector
{
  public:
    ...
    inline void GetNextObject( gmGCObjBase* a_obj )
    {
        m_colorSet.GrayAWhite( a_obj );
    }

    /// \brief Called on a new object being allocated
    inline void AllocateObject( gmGCObjBase* a_obj )
    {
        m_colorSet.Allocate( a_obj );
    }

    /// \brief Get the current shade color since it is flipped each cycle.
    inline int GetCurShadeColor()
    {
        return ( m_curShadeColor );
    }

    /// \brief Is the object colored gray or black?
    inline bool IsShaded( gmGCObjBase* a_obj )
    {
        return ( a_obj->GetColor() == m_curShadeColor );
    }
    ...
    inline gmGCTraceState& GetTraceState()
    {
        return m_traceState;
    }

    /// \brief Set the amount of work to do per increment of collecting
    inline void SetWorkPerIncrement( int a_workPerIncrement )
    {
        m_workPerIncrement = a_workPerIncrement;
    }
    /// \brief Set the amount of objects to destruct per increment of collecting
    inline void SetDestructPerIncrement( int a_destructPerIncrement )
    {
        m_maxObjsToDestructPerIncrement = a_destructPerIncrement;
    }

    /// \brief Get the amount of work to do per increment of collecting
    inline int GetWorkPerIncrement()
    {
        return m_workPerIncrement;
    }
    /// \brief Get the amount of objects to destruct per increment of collecting
    inline int GetDestructPerIncrement()
    {
        return m_maxObjsToDestructPerIncrement;
    }

    /// \brief Set function to be called before flip when dead objects are reclaimed. 
    /// Optional, so pass NULL to disable.
    inline void SetFlipCallback( GCFlipCallBack a_flipCallback )
    {
        m_flipCallback = a_flipCallback;
    }

    /// \brief Is collector currently turned off?
    inline bool IsOff()
    {
        return m_gcTurnedOff;
    }
    ...
    {
        return m_colorSet.DestructSomeFreeObjects( m_maxObjsToDestructPerIncrement );
    }

    /// \brief Make an object persistant by moving it into the persistant list.
    void MakeObjectPersistant( gmGCObjBase* a_obj )
    {
        m_colorSet.MakePersistant( a_obj );
    }

    /// \brief Get the virtual machine for language
    inline gmMachine* GetVM()
    {
        return m_gmMachine;
    }
    ...
    gmObject* CheckReference( gmptr a_ref )
    {
        return m_colorSet.CheckReference( a_ref );
    }

    /// \brief Get instruction at point for VM Debugger.
    const void* GetInstructionAtBreakPoint( gmuint32 a_sourceId, int a_line )
    {
        return m_colorSet.GetInstructionAtBreakPoint( a_sourceId, a_line );
    }

  protected:

    /// \brief Has Collect() completed yet?
    inline bool IsDoneTracing()
    {
        return m_doneTracing;
    }
    ...
    inline void ToggleCurShadeColor()
    {
        m_curShadeColor = !m_curShadeColor;
    }
