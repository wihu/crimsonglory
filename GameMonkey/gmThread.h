...
class gmThread : public gmListDoubleNode<gmThread>, public gmHashNode<int, gmThread>
{
  public:
    ...
    enum State { RUNNING = 0, SLEEPING, BLOCKED, KILLED, EXCEPTION, //!< exception state, for debugging

    SYS_PENDING, SYS_YIELD, SYS_EXCEPTION, };
    ...
    inline gmVariable* GetTop()
    {
        return &m_stack[m_top];
    }

    /// \brief GetBottom() will return the bottom of stact
    inline gmVariable* GetBottom()
    {
        return m_stack;
    }
...
{
    m_stack[m_top].m_type = GM_STRING;
    return ( gmStringObject * )
           ( m_stack[m_top++].m_value.m_ref = ( gmptr ) m_machine->AllocStringObject( a_value, a_len ) );
}
...
gmUserObject* gmThread::PushNewUser( void* a_user, int a_userType )
{
    ...
    return ( gmUserObject * )
           ( m_stack[m_top++].m_value.m_ref = ( gmptr ) m_machine->AllocUserObject( a_user, a_userType ) );
}
