...
namespace irr
{
    ...
    {
        /** Constructor*/
        CDMFLoader::CDMFLoader( video::IVideoDriver* driver, scene::ISceneManager* smgr ) : Driver( driver ), SceneMgr( smgr )
        {
#ifdef _DEBUG
        ...
        IAnimatedMesh* CDMFLoader::createMesh( irr::io::IReadFile* file, vector3df vScale )
        {
            ...
            if ( GetDMFHeader( dmfRawFile, header ) )
            {
              ...
              if ( GetDMFSprites( dmfRawFile, vScale ) )
              {
              }

              dmfLight* lights = new dmfLight[header.numLights];
              if ( GetDMFLights( dmfRawFile, lights ) )
              {
                for ( i = 0; i < header.numLights; i++ )
                {
                  IRR.smgr->addLightSceneNode( 0, vector3df( lights[i].pos[0] * vScale.X, lights[i].pos[1] * vScale.Y, lights[i].pos[2] * vScale.Z ), lights[i].diffuseColor, lights[i].radius );
                }
              }
              ...
              for ( i = 0; i < header.numFaces; i++ )
              {
                ...
                SMeshBufferLightMap* meshBuffer = ( SMeshBufferLightMap* )Mesh->getMeshBuffer( faces[i].materialID );
                ...
                for ( v = 0; v < faces[i].numVerts; v++ )
                {
                  ...
                  if ( materiali[faces[i].materialID].textureBlend == 4 && SceneMgr->getParameters()->getAttributeAsBool( DMF_FLIP_ALPHA_TEXTURES ) )
                  {
              ...
              for ( i = 0; i < header.numMaterials; i++ )
              {
                String path;
                path = "";
                if ( SceneMgr->getParameters()->getAttributeAsString( DMF_TEXTURE_PATH ).size() == 0 )
                {
                ...
                {
                  path = path + String( SceneMgr->getParameters()->getAttributeAsString( DMF_TEXTURE_PATH ) ) + String( "\\" );
                }
				...
				path = "Textures/";
                tex = Driver->getTexture( APP.useFile( ( path + String( materiali[i].textureName ) ).c_str() ).c_str() );
                ...
                if ( materiali[i].textureFlag == 1 )
                {
                  ...
                  SColor color = SColor( axtoi( alpha.c_str() ), axtoi( red.c_str() ), axtoi( green.c_str() ), axtoi( blue.c_str() ) );
                  ...
                  IImage* immagine = Driver->createImageFromData( ECF_A8R8G8B8, core::dimension2d<s32>( 8, 8 ), buf );
                ...
                {
                  lig = Driver->getTexture( APP.useFile( ( path + String( materiali[i].lightmapName ) ).c_str() ).c_str() );
                }
                else //no lightmap
                {
                  lig = 0;
                  ...
                  mult = mult / 100.0f;
                  buffer->Material.AmbientColor = header.dmfAmbient.getInterpolated( SColor( 255, 255, 255, 255 ), mult );
                }       
            ...
            {
              if ( Mesh->MeshBuffers[i]->getVertexCount() == 0 || Mesh->MeshBuffers[i]->getIndexCount() == 0 || Mesh->MeshBuffers[i]->getMaterial().Texture1 == 0 )
              {
