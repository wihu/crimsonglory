...
CReflectedWater::CReflectedWater( const irr::c8* name, IrrlichtDevice* device, scene::ISceneManager* smgr, s32 id, f32 tileSize, s32 tileCount, core::dimension2d<s32> RenderTargetSize )
{
    m_device = device; 
    m_smgr = smgr; 
    m_name = name; 
    ...
    // Init constants 
    m_AddedColor = video::SColor( 255, 1, 1, 1 ); 
    m_MultiColor = video::SColor( 255, 233, 233, 233 ); 
    m_WaveHeight = 3.0f; 
    m_WaveLength = 50.0f; 
    m_WaveSpeed = 10.0f; 
    m_WaveDisplacement = 7.0f; 
    m_WaveRepetition = 5.0f; 
    m_RefractionFactor = 1.0f; 

    scene::IAnimatedMesh* wmesh;
    smgr->addHillPlaneMesh( m_name, core::dimension2d<f32>( tileSize, tileSize ), core::dimension2d<s32>( tileCount, tileCount ) ); 
    m_waternode = smgr->addMeshSceneNode( smgr->getMesh( m_name )->getMesh( 0 ) ); 
    if ( !m_waternode )
    {
      return;
    } 
    m_waternode->setRotation( vector3df( 180, 0, 0 ) );
    ...
    {
      dmat = gpu->addHighLevelShaderMaterial( WATER_VERTEX_GLSL, "main", video::EVST_VS_1_1, WATER_FRAGMENT_GLSL, "main", video::EPST_PS_1_1, this, video::EMT_SOLID, 0 );
    }
    else
    {
      dmat = gpu->addHighLevelShaderMaterial( WATER_HLSL, "vertexMain", video::EVST_VS_2_0, WATER_HLSL, "pixelMain", video::EPST_PS_2_0, this, video::EMT_SOLID, 2 );
    } 

    m_waternode->setMaterialType( ( video::E_MATERIAL_TYPE )dmat ); 
    m_waternode->setMaterialFlag( video::EMF_BACK_FACE_CULLING, false ); 
    m_waternode->setMaterialFlag( video::EMF_LIGHTING, ( bool )IRR.useLighting ); 
    m_waternode->setMaterialFlag( video::EMF_FOG_ENABLE, true ); 

    m_rt = _driver->createRenderTargetTexture( RenderTargetSize ); 
    m_waternode->setMaterialTexture( 0, m_rt ); 

    //m_waternode->setMaterialTexture(1, IRR.video->getTexture( APP.useFile( "Textures/natural_water.jpg").c_str() ) );
...
void CReflectedWater::updateRendertarget( scene::ISceneManager* smgr )
{
    ...
    m_watercam->setFOV( camera->getFOV() );
    ...
    if ( campos.Y < Position.Y )
    {
      ...
      target.normalize(); 
      target.Y *= -1; 
      m_watercam->setTarget( m_watercam->getPosition() + target * 200000 ); 
