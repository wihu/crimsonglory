...
static int GM_CDECL gmSleep( gmThread* a_thread ) // float\int param time in seconds
{
    ...
    if ( type == GM_INT )
    {
      ms = a_thread->Param( 0 ).m_value.m_int * 1000;
    }
    else if ( type == GM_FLOAT )
    {
      ms = ( gmuint32 )floorf( a_thread->Param( 0 ).m_value.m_float * 1000.0f );
    }
...
static bool GM_CDECL gmGCTraceStateUserType( gmMachine* a_machine, gmUserObject* a_object, gmGarbageCollector* a_gc, const int a_workLeftToGo, int& a_workDone )
{
    ...
    {
      a_gc->GetNextObject( state->m_currentState );
    }
    if ( state->m_lastState )
    {
      a_gc->GetNextObject( state->m_lastState );
    }
    if ( state->m_setExitState )
    {
      a_gc->GetNextObject( state->m_setExitState );
    }
...
{
    gmStateUserType* state = ( gmStateUserType* )a_object->m_user;
    if ( state->m_currentState && state->m_currentState->NeedsMark( a_mark ) )
    {
      state->m_currentState->Mark( a_machine, a_mark );
    }
    if ( state->m_lastState && state->m_lastState->NeedsMark( a_mark ) )
    {
      state->m_lastState->Mark( a_machine, a_mark );
    }
    if ( state->m_setExitState && state->m_setExitState->NeedsMark( a_mark ) )
    {
      state->m_setExitState->Mark( a_machine, a_mark );
    }
}
