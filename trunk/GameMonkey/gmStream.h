...
class gmStream
{
  public:
    ...
    enum Flags { F_EOS = ( 1 << 0 ), ///< is the stream at End Of Stream
    ...
    inline void SetSwapEndianOnWrite( bool a_state )
    {
        m_swapEndian = a_state;
    }
    inline bool GetSwapEndianOnWrite() const
    {
        return m_swapEndian;
    }
    ...
    inline void SetEndianOnWrite( gmEndian a_endian )
    {
        ...
        if ( ( ( a_endian == GM_ENDIAN_LITTLE ) && !isLittle ) || ( ( a_endian == GM_ENDIAN_BIG ) && isLittle ) )
        {
            needSwap = true;
        }
  ...
  private:
    ...
    inline void SwapEndian( gmuint16& a_x )
    {
        a_x = ( gmuint16 ) ( ( a_x << 8 ) | ( ( a_x >> 8 ) & 0xff ) );
    }
    inline void SwapEndian( gmint16& a_x )
    {
        a_x = ( gmint16 ) ( ( a_x << 8 ) | ( ( a_x >> 8 ) & 0xff ) );
    }
    inline void SwapEndian( gmuint32& a_x )
    {
        a_x = ( a_x << 24 )
            | ( ( a_x << 8 ) & 0x00ff0000 )
            | ( ( a_x >> 8 ) & 0x0000ff00 )
    ...
    inline void SwapEndian( gmint32& a_x )
    {
        a_x = ( a_x << 24 )
            | ( ( a_x << 8 ) & 0x00ff0000 )
            | ( ( a_x >> 8 ) & 0x0000ff00 )
    ...
    inline void SwapEndian( gmfloat& a_x )
    {
        SwapEndian( ( gmuint32 & ) a_x );
    }
};
