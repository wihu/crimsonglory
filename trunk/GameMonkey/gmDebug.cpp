...
enum gmdThreadFlags { TF_STEPOVER = ( 1 << 0 ), TF_STEPINTO = ( 1 << 1 ), TF_STEPOUT = ( 1 << 2 ), TF_BREAK = ( 1 << 3 ), };
...
static bool LineCallback( gmThread* a_thread )
{
    ...
    if ( !( a_thread->m_debugFlags & TF_STEPOVER ) || ( a_thread->m_debugUser != ( ( a_thread->GetFrame() ) ? a_thread->GetFrame()->m_returnBase : 0 ) ) )
    {
...
static bool RetCallback( gmThread* a_thread )
{
    gmDebugSession* session = ( gmDebugSession* )a_thread->GetMachine()->m_debugUser;
    GM_ASSERT( session );
    if ( ( ( a_thread->m_debugFlags & TF_STEPOUT ) && ( a_thread->m_debugUser == a_thread->GetIntBase() ) ) || ( ( a_thread->m_debugFlags & TF_STEPOVER ) && ( a_thread->m_debugUser == a_thread->GetIntBase() ) ) )
    {
      a_thread->m_debugFlags = TF_BREAK;
      const gmFunctionObject* fn = a_thread->GetFunctionObject();
      gmDebuggerBreak( session, a_thread->GetId(), fn->GetSourceId(), fn->GetLine( a_thread->GetInstruction() ) );
...
bool GM_CDECL gmdMachineCallback( gmMachine* a_machine, gmMachineCommand a_command, const void* a_context )
{
    ...
    // chain callback
    if ( s_prevMachineCallback )
    {
      s_prevMachineCallback( a_machine, a_command, a_context );
    }
...
gmDebugSession::gmDebugSession() : m_breaks( 32 )
{
    m_machine = NULL;
}
