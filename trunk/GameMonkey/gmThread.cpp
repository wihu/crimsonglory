        ...
        case BC_OP_NOT :
          {
              ...
              else if ( ( fn = CALLOPERATOR( operand->m_type, ( gmOperator )instruction32[-1] ) ) )
              {
                ...
                if ( res == SYS_EXCEPTION )
                {
                  goto LabelException;
                }
                if ( res == KILLED )
                {
                  m_machine->Sys_SwitchState( this, KILLED ); GM_ASSERT( 0 );
                } // operator should not kill a thread
                return res;
              }
              else
              {
