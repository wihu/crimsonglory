...
enum { GM_NULL = 0, // GM_NULL must be 0 as i have relied on this in expression testing.
GM_INT, GM_FLOAT, GM_STRING, GM_TABLE, GM_FUNCTION, GM_USER,     // User types continue from here.
...
struct gmVariable
{
    ...
    inline gmVariable( gmType a_type, gmptr a_ref ) : m_type( a_type )
    {
        m_value.m_ref = a_ref;
    }

    explicit inline gmVariable( int a_val ) : m_type( GM_INT )
    {
        m_value.m_int = a_val;
    }
    explicit inline gmVariable( float a_val ) : m_type( GM_FLOAT )
    {
        m_value.m_float = a_val;
    }
    explicit inline gmVariable( gmStringObject* a_string )
    {
        SetString( a_string );
    }
    explicit inline gmVariable( gmTableObject* a_table )
    {
        SetTable( a_table );
    }
    explicit inline gmVariable( gmFunctionObject* a_func )
    {
        SetFunction( a_func );
    }
    explicit inline gmVariable( gmUserObject* a_user )
    {
        SetUser( a_user );
    }

    inline void SetInt( int a_value )
    {
        m_type = GM_INT; m_value.m_int = a_value;
    }
    inline void SetFloat( float a_value )
    {
        m_type = GM_FLOAT; m_value.m_float = a_value;
    }
    ...
    {
        return m_type > GM_FLOAT;
    }
    inline void Nullify()
    {
        m_type = GM_NULL; m_value.m_int = 0;
    }
    ...
    {
        if ( a_keyA.m_type < a_keyB.m_type )
        {
            return -1;
        }
        if ( a_keyA.m_type > a_keyB.m_type )
        ...
        if ( a_keyA.m_value.m_int < a_keyB.m_value.m_int )
        {
            return -1;
        }
        if ( a_keyA.m_value.m_int > a_keyB.m_value.m_int )
...
#endif //GM_USE_INCGC
{
  public:
    ...
    inline bool NeedsMark( gmuint32 a_mark )
    {
        return ( ( m_mark != GM_MARK_PERSIST ) && ( m_mark != a_mark ) );
    }
