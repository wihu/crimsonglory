...
class gmCallScript
{
  public:
    ...
    GM_FORCEINLINE static bool BeginGlobal( const char* a_funcName, bool a_delayExecuteFlag = false,
    ...
    /// \return true on sucess, false if function was not found.
    GM_FORCEINLINE static bool BeginGlobal( gmStringObject* a_funcNameStringObj, bool a_delayExecuteFlag = false,
                                            gmVariable* a_this = NULL )
    {
        GM_ASSERT( s_machine );
    ...
    /// \return true on sucess, false if function was not found.
    GM_FORCEINLINE static bool BeginObject( const char* a_funcName, gmTableObject* a_tableObj, gmUserObject* a_userObj,
    ...
    /// \param a_userObj The 'this' pointer used by the function.
    /// \return true on sucess, false if function was not found.
    GM_FORCEINLINE static bool BeginObject( gmStringObject* a_funcNameStringObj, gmTableObject* a_tableObj,
                                            gmUserObject* a_userObj, bool a_delayExecuteFlag = false )
    {
        GM_ASSERT( s_machine );   
    ...
    /// \param a_userObj The 'this' pointer used by the function.
    /// \return true on sucess, false if function was not found.
    GM_FORCEINLINE static bool BeginFunction( gmFunctionObject* a_funcObj, const gmVariable& a_thisVar,
                                              bool a_delayExecuteFlag = false )
    {
        GM_ASSERT( s_machine );   
        GM_ASSERT( a_funcObj );
        ...
        {
            s_thread = s_machine->CreateThread();     // Create thread for func to run on      
            s_thread->Push( a_thisVar );                // this
            s_thread->PushFunction( a_funcObj );        // function
            s_delayExecuteFlag = a_delayExecuteFlag;
            return true;
        }
    ...
    GM_FORCEINLINE static void AddParamTable( gmTableObject* a_tableObj )
    {
        GM_ASSERT( s_machine );
        GM_ASSERT( s_thread );

        s_thread->PushTable( a_tableObj );
        ++s_paramCount;
    }
    ...
    GM_FORCEINLINE static void End( int* a_threadId = NULL )
    {
        ...
        else
        {
            ...
            s_machine->Sys_SwitchState( s_thread, gmThread::KILLED );
        }
    ...
    static GM_FORCEINLINE gmThread* Thread()
    {
        return s_thread;
    }
