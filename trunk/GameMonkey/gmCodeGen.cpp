...
bool gmCodeGenPrivate::Generate( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode, bool a_siblings )
{
    ...
    while ( a_node )
    {
      ...
      if ( m_debug && ( s_line != a_node->m_lineNumber ) && !( a_node->m_type == CTNT_STATEMENT && a_node->m_subType == CTNST_COMPOUND ) )
      {
        ...
        case CTNT_DECLARATION :
          {
              ...
              {
                case CTNDT_VARIABLE :
                  res = GenDeclVariable( a_node, a_byteCode ); break;
                default:
                  {
                      GM_ASSERT( false );
                      return false;
        ...
        case CTNT_STATEMENT :
          {
              ...
              {
                case CTNST_RETURN :
                  res = GenStmtReturn( a_node, a_byteCode ); break;
                case CTNST_BREAK :
                  res = GenStmtBreak( a_node, a_byteCode ); break;
                case CTNST_CONTINUE :
                  res = GenStmtContinue( a_node, a_byteCode ); break;
                case CTNST_FOR :
                  res = GenStmtFor( a_node, a_byteCode ); break;
                case CTNST_FOREACH :
                  res = GenStmtForEach( a_node, a_byteCode ); break;
                case CTNST_WHILE :
                  res = GenStmtWhile( a_node, a_byteCode ); break;
                case CTNST_DOWHILE :
                  res = GenStmtDoWhile( a_node, a_byteCode ); break;
                case CTNST_IF :
                  res = GenStmtIf( a_node, a_byteCode ); break;
                case CTNST_COMPOUND :
                  res = GenStmtCompound( a_node, a_byteCode ); break;
                default:
                  {
                      GM_ASSERT( false );
                      return false;
        ...
        case CTNT_EXPRESSION :
          {
              switch ( a_node->m_subType )
              {
                case CTNET_OPERATION :
                  {
                      ...
                      {
                        case CTNOT_DOT :
                          res = GenExprOpDot( a_node, a_byteCode ); break;
                        case CTNOT_UNARY_PLUS :
                        case CTNOT_UNARY_MINUS :
                        case CTNOT_UNARY_COMPLEMENT :
                        case CTNOT_UNARY_NOT :
                          res = GenExprOpUnary( a_node, a_byteCode ); break;
                        case CTNOT_ARRAY_INDEX :
                          res = GenExprOpArrayIndex( a_node, a_byteCode ); break;
                        case CTNOT_TIMES :
                        case CTNOT_DIVIDE :
                        case CTNOT_REM :
                        case CTNOT_MINUS :
                        case CTNOT_ADD :
                          res = GenExprOpAr( a_node, a_byteCode ); break;
                        case CTNOT_SHIFT_LEFT :
                        case CTNOT_SHIFT_RIGHT :
                          res = GenExprOpShift( a_node, a_byteCode ); break;
                        case CTNOT_LT :
                        case CTNOT_GT :
                        case CTNOT_LTE :
                        case CTNOT_GTE :
                        case CTNOT_EQ :
                        case CTNOT_NEQ :
                          res = GenExprOpComparison( a_node, a_byteCode ); break;
                        case CTNOT_BIT_AND :
                        case CTNOT_BIT_XOR :
                        case CTNOT_BIT_OR :
                          res = GenExprOpBitwise( a_node, a_byteCode ); break;
                        case CTNOT_AND :
                          res = GenExprOpAnd( a_node, a_byteCode ); break;
                        case CTNOT_OR :
                          res = GenExprOpOr( a_node, a_byteCode ); break;
                        case CTNOT_ASSIGN :
                          res = GenExprOpAssign( a_node, a_byteCode ); break;
                        default:
                          {
                              GM_ASSERT( false );
                              return false;
                ...
                case CTNET_CONSTANT :
                  res = GenExprConstant( a_node, a_byteCode ); break;
                case CTNET_IDENTIFIER :
                  res = GenExprIdentifier( a_node, a_byteCode ); break;
                case CTNET_CALL :
                  res = GenExprCall( a_node, a_byteCode ); break;
                case CTNET_THIS :
                  res = GenExprThis( a_node, a_byteCode ); break;
                case CTNET_FUNCTION :
                  res = GenExprFunction( a_node, a_byteCode ); break;
                case CTNET_TABLE :
                  res = GenExprTable( a_node, a_byteCode ); break;
                default :
                  {
                      GM_ASSERT( false );
                      return false;
...
bool gmCodeGenPrivate::GenExprFunction( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    if ( m_debug && a_node->m_parent && a_node->m_parent->m_type == CTNT_EXPRESSION && a_node->m_parent->m_subType == CTNET_OPERATION && ( a_node->m_parent->m_subTypeType == CTNOT_ASSIGN || a_node->m_parent->m_subTypeType == CTNOT_ASSIGN_FIELD ) && a_node->m_parent->m_children[1] == a_node )
    {
    ...
    while ( params )
    {
      ...
      if ( m_currentFunction->SetVariableType( param->m_data.m_string, CTVT_LOCAL ) != numParams )
      {
        if ( m_log )
        {
          m_log->LogEntry( "error (%d) parameter %s already declared", param->m_lineNumber, param->m_data.m_string );
        }
        PopFunction();
        return false;
      }
...
bool gmCodeGenPrivate::GenExprTable( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    while ( fields )
    {
      ...
      {
        if ( !Generate( fields->m_children[1], a_byteCode ) )
        {
          return false;
        }
      ...
      else
      {
        ...
        if ( !Generate( fields, a_byteCode, false ) )
        {
          return false;
        }
...
bool gmCodeGenPrivate::GenStmtBreak( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    if ( m_log )
    {
      m_log->LogEntry( "error (%d) illegal break statement", a_node->m_lineNumber );
    }
    return false;
}
...
bool gmCodeGenPrivate::GenStmtContinue( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    if ( m_log )
    {
      m_log->LogEntry( "error (%d) illegal continue statement", a_node->m_lineNumber );
    }
    return false;
}
...
bool gmCodeGenPrivate::GenStmtForEach( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      keyVar = a_node->m_children[2]->m_data.m_string;
    }
...
bool gmCodeGenPrivate::GenExprOpDot( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    if ( m_log )
    {
      m_log->LogEntry( "error (%d) illegal dot operator", a_node->m_lineNumber );
    }
    return false;
}
...
bool gmCodeGenPrivate::GenExprOpUnary( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      case CTNOT_UNARY_PLUS :
        return a_byteCode->Emit( BC_OP_POS );
      case CTNOT_UNARY_MINUS :
        return a_byteCode->Emit( BC_OP_NEG );
      case CTNOT_UNARY_NOT :
        return a_byteCode->Emit( BC_OP_NOT );
      case CTNOT_UNARY_COMPLEMENT :
        return a_byteCode->Emit( BC_BIT_INV );
      default :
        {
            if ( m_log )
            {
              m_log->LogEntry( "error (%d) unkown operator", a_node->m_lineNumber );
            }
        }
    }
    return false;
...
bool gmCodeGenPrivate::GenExprOpAr( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      case CTNOT_TIMES :
        return a_byteCode->Emit( BC_OP_MUL );
      case CTNOT_DIVIDE :
        return a_byteCode->Emit( BC_OP_DIV );
      case CTNOT_REM :
        return a_byteCode->Emit( BC_OP_REM );
      case CTNOT_ADD :
        return a_byteCode->Emit( BC_OP_ADD );
      case CTNOT_MINUS :
        return a_byteCode->Emit( BC_OP_SUB );
      default :
        {
            if ( m_log )
            {
              m_log->LogEntry( "error (%d) unkown arithmatic operator", a_node->m_lineNumber );
            }
        }
    }
    return false;
...
bool gmCodeGenPrivate::GenExprOpShift( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      case CTNOT_SHIFT_LEFT :
        return a_byteCode->Emit( BC_BIT_SHL );
      case CTNOT_SHIFT_RIGHT :
        return a_byteCode->Emit( BC_BIT_SHR );
      default :
        {
            if ( m_log )
            {
              m_log->LogEntry( "error (%d) unkown shift operator", a_node->m_lineNumber );
            }
        }
    }
    return false;
...
bool gmCodeGenPrivate::GenExprOpComparison( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      case CTNOT_LT :
        return a_byteCode->Emit( BC_OP_LT );
      case CTNOT_GT :
        return a_byteCode->Emit( BC_OP_GT );
      case CTNOT_LTE :
        return a_byteCode->Emit( BC_OP_LTE );
      case CTNOT_GTE :
        return a_byteCode->Emit( BC_OP_GTE );
      case CTNOT_EQ :
        return a_byteCode->Emit( BC_OP_EQ );
      case CTNOT_NEQ :
        return a_byteCode->Emit( BC_OP_NEQ );
      default :
        {
            if ( m_log )
            {
              m_log->LogEntry( "error (%d) unkown comparison operator", a_node->m_lineNumber );
            }
        }
    }
    return false;
...
bool gmCodeGenPrivate::GenExprOpBitwise( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      case CTNOT_BIT_AND :
        return a_byteCode->Emit( BC_BIT_AND );
      case CTNOT_BIT_XOR :
        return a_byteCode->Emit( BC_BIT_XOR );
      case CTNOT_BIT_OR  :
        return a_byteCode->Emit( BC_BIT_OR );
      default :
        {
            if ( m_log )
            {
              m_log->LogEntry( "error (%d) unkown bitwise operator", a_node->m_lineNumber );
            }
        }
    }
    return false;
...
bool gmCodeGenPrivate::GenExprOpAssign( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    else
    {
      if ( m_log )
      {
        m_log->LogEntry( "error (%d) illegal l-value for '=' operator", a_node->m_lineNumber );
      }
      return false;
    }
      ...
      default:
        {
            if ( m_log )
            {
              m_log->LogEntry( "unkown constant type" );
            }
            return false;
        }
    }
...
bool gmCodeGenPrivate::GenExprCall( const gmCodeTreeNode* a_node, gmByteCodeGen* a_byteCode )
{
    ...
    {
      if ( !Generate( callee->m_children[0], a_byteCode ) )
      {
        return false;
      }
      a_byteCode->Emit( BC_DUP );
    ...
    {
      ++numParams;
      if ( !Generate( params, a_byteCode, false ) )
      {
        return false;
      }
      params = params->m_sibling;
