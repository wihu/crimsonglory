...
gmCodeTree::gmCodeTree() : m_mem( 1, GMCODETREE_CHAINSIZE )
{
    g_codeTree = NULL;
    m_locked = false;
    m_errors = 0;
...
const char* gmGetOperatorTypeName( gmCodeTreeNodeOperationType a_type )
{
    ...
    {
      case CTNOT_INVALID :
        return "CTNOT_INVALID";
      case CTNOT_DOT :
        return "CTNOT_DOT";
      case CTNOT_UNARY_PLUS :
        return "CTNOT_UNARY_PLUS";
      case CTNOT_UNARY_MINUS :
        return "CTNOT_UNARY_MINUS";
      case CTNOT_UNARY_NOT :
        return "CTNOT_UNARY_NOT";
      case CTNOT_UNARY_COMPLEMENT :
        return "CTNOT_UNARY_COMPLEMENT";
      case CTNOT_ARRAY_INDEX :
        return "CTNOT_ARRAY_INDEX";
      case CTNOT_TIMES :
        return "CTNOT_TIMES";
      case CTNOT_DIVIDE :
        return "CTNOT_DIVIDE";
      ...
      case CTNOT_BIT_OR :
        return "CTNOT_BIT_OR";
      case CTNOT_BIT_XOR :
        return "CTNOT_BIT_XOR";
      case CTNOT_BIT_AND :
        return "CTNOT_BIT_AND";
      case CTNOT_SHIFT_LEFT :
        return "CTNOT_SHIFT_LEFT";
      case CTNOT_SHIFT_RIGHT :
        return "CTNOT_SHIFT_RIGHT";
      case CTNOT_ASSIGN :
        return "CTNOT_ASSIGN";
      case CTNOT_ASSIGN_FIELD :
        return "CTNOT_ASSIGN_FIELD";
      default :
        break;
    }
    ...
    if ( a_node )
    {
      ...
      while ( a_node != NULL )
      {
        ...
        if ( a_node->m_type == CTNT_DECLARATION )
        {
          ...
          switch ( a_node->m_subType )
          {
            case CTNDT_PARAMETER :
              fprintf( a_fp, "CTNDT_PARAMETER:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNDT_VARIABLE :
              fprintf( a_fp, "CTNDT_VARIABLE:%04d, type %d"GM_NL, a_node->m_lineNumber, a_node->m_subTypeType ); break;
            default :
              fprintf( a_fp, "UNKNOWN DECLARATION:"GM_NL ); break;
          }
        }
        else if ( a_node->m_type == CTNT_STATEMENT )
        {
          ...
          {
            case CTNST_RETURN :
              fprintf( a_fp, "CTNST_RETURN:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_BREAK :
              fprintf( a_fp, "CTNST_BREAK:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_CONTINUE :
              fprintf( a_fp, "CTNST_CONTINUE:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_FOR :
              fprintf( a_fp, "CTNST_FOR:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_FOREACH :
              fprintf( a_fp, "CTNST_FOREACH:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_WHILE :
              fprintf( a_fp, "CTNST_WHILE:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_DOWHILE :
              fprintf( a_fp, "CTNST_DOWHILE:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_IF :
              fprintf( a_fp, "CTNST_IF:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNST_COMPOUND :
              fprintf( a_fp, "CTNST_COMPOUND:%04d"GM_NL, a_node->m_lineNumber ); break;
            default :
              fprintf( a_fp, "UNKNOWN STATEMENT:"GM_NL ); break;
          }
        }
            ...
            case CTNET_CONSTANT :
              {
                  ...
                  {
                    case CTNCT_INT :
                      fprintf( a_fp, "CTNCT_INT:%04d : %d"GM_NL, a_node->m_lineNumber, a_node->m_data.m_iValue ); break;
                    case CTNCT_FLOAT :
                      fprintf( a_fp, "CTNCT_FLOAT:%04d : %f"GM_NL, a_node->m_lineNumber, a_node->m_data.m_fValue ); break;
                    case CTNCT_STRING :
                      fprintf( a_fp, "CTNCT_STRING:%04d : %s"GM_NL, a_node->m_lineNumber, a_node->m_data.m_string ); break;
                    case CTNCT_NULL :
                      fprintf( a_fp, "CTNCT_NULL:%04d"GM_NL, a_node->m_lineNumber ); break;
                    default:
                      fprintf( a_fp, "UNKNOWN CTNET_CONSTANT"GM_NL );
                  }
                  break;
              }

            case CTNET_IDENTIFIER :
              fprintf( a_fp, "CTNET_IDENTIFIER:%04d : %s"GM_NL, a_node->m_lineNumber, a_node->m_data.m_string ); break;
            case CTNET_THIS :
              fprintf( a_fp, "CTNET_THIS:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNET_CALL :
              fprintf( a_fp, "CTNET_CALL:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNET_FUNCTION :
              fprintf( a_fp, "CTNET_FUNCTION:%04d"GM_NL, a_node->m_lineNumber ); break;
            case CTNET_TABLE :
              fprintf( a_fp, "CTNET_TABLE:%04d"GM_NL, a_node->m_lineNumber ); break;
            default :
              fprintf( a_fp, "UNKNOWN EXPRESSION:"GM_NL ); break;
          }
        }
        else
        {
...
bool gmCodeTreeNode::ConstantFold()
{
    if ( m_type == CTNT_EXPRESSION && m_subType == CTNET_OPERATION )
    {
      ...
      else if ( possibleFold )
      {
        ...
        if ( ( l && l->m_type == CTNT_EXPRESSION && l->m_subType == CTNET_CONSTANT ) && ( r && r->m_type == CTNT_EXPRESSION && r->m_subType == CTNET_CONSTANT ) )
        {
          if ( ( l->m_subTypeType == CTNCT_INT || ( l->m_subTypeType == CTNCT_FLOAT && !intOnly ) ) && ( r->m_subTypeType == CTNCT_INT || ( r->m_subTypeType == CTNCT_FLOAT && !intOnly ) ) )
          {
            // we can fold....
