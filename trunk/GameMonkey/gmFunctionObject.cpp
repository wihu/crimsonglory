...
void gmFunctionObject::Destruct( gmMachine* a_machine )
{
    ...
    if ( m_debugInfo )
    {
      if ( m_debugInfo->m_debugName )
      {
        a_machine->Sys_Free( m_debugInfo->m_debugName );
      }
      if ( m_debugInfo->m_lineInfo )
      {
        a_machine->Sys_Free( m_debugInfo->m_lineInfo );
      }
...
void gmFunctionObject::Mark( gmMachine* a_machine, gmuint32 a_mark )
{
    ...
    {
      gmObject* object = a_machine->GetObject( m_references[i] );
      if ( object->NeedsMark( a_mark ) )
      {
        object->Mark( a_machine, a_mark );
      }
    }
}
#endif //GM_USE_INCGC
    ...
    if ( m_byteCode )
    {
      ...
      for ( ; instruction < end; )
      {
        switch ( *( instruction32++ ) )
        {
          ...
          case BC_GETTHIS :
          case BC_SETTHIS :
            instruction += sizeof( gmptr ); break;
          case BC_PUSHFP :
          ...
          case BC_PUSHFN :
            {
                ...
                if ( i == m_numReferences )
                {
                  references[m_numReferences++] = reference;
                }
                break;
            }
