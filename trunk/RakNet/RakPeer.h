...
int PlayerIDAndIndexComp( const PlayerID &key, const PlayerIDAndIndex &data ); // GCC requires RakPeer::PlayerIDAndIndex or it won't compile
...
/// \note This class is not safe to call from multiple threads at the same time.  For a multithreaded app, either modify the code on your own to make it thread-safe (put mutexes around the SingleProducerConsumer members), limit your RakNet calls to one thread, or limit function calls to one thread and send the output to worker threads.
...
public:
	...
	/// \param[in] blockDuration How long you should wait for all remaining packets to go out.  If 0, it doesn't wait at all.
...
protected:
	...
	///Local Player ID
	PlayerID myPlayerId;
	char incomingPassword[256], outgoingPassword[256];
	unsigned char incomingPasswordLength, outgoingPasswordLength;
...
#ifndef _FINAL
	unsigned short _maxPacketsOnWire, _minExtraPing, _extraPingVariance;
#endif
